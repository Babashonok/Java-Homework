         ...
         <table  >
             <%
                 ...
                 while (it.hasNext()) {
                     ...
                     try {
                         key = Integer.parseInt(entry.getKey()) + 1;
                     }catch(Exception ex){}
             %>
               <tr>
                   <td ><%=entry.getValue()%></td>
                   <td width="10%"></td>
                   <td ><a href="/?action=edit_phone&id=<%=person.getId()%>&phone_id=<%=entry.getKey()%>">Редактировать</a></td>
                   <td width="10%"></td>
                   <td ><a href="/?action=delete_phone&id=<%=person.getId()%>&phone_id=<%=entry.getKey()%>">Удалить</a></td>
               </tr>
             <%}%>
             <tr>
                 <td>
                     <a href="/?action=add_phone&id=<%=person.getId()%>&phone_id=<%=key%>">Добавить</a>
                 </td>
             </tr>
         </table>
        </td>
